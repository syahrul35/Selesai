...
export default function TaskModal({ show, onClose, task, projects = [] }) {
    ...
    const normalizeDateTime = (value) => {
        ...
        const dateTime = value.toString().split(".")[0]; // Remove milliseconds
        return dateTime;
    };
    ...
    useEffect(() => {
        if (task) {
            ...
            setData({
                ...
                assigned_to: task.assigned_to || "",
                due_at: normalizeDateTime(task.due_at),
                time_notif: normalizeDateTime(task.time_notif) || "18:00",
                priority: task.priority || "medium",
                description: task.description || "",
            });
        } else {
    ...
    const handleSubmit = (e) => {
        ...
        const submitData = {
            ...
            project_id:
                data.project_id === "no_project" ? null : data.project_id,
            // If assigned_to is empty, send null
            assigned_to: data.assigned_to || null,
        };
        
        // if not editing a task, delete due_at form
        if (!task) {
            delete submitData.due_at;
        }
            ...
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 overflow-y-auto max-h-[90vh]">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            setData("time_notif", e.target.value)
                        }
                        className="w-full border rounded px-3 py-2"
                    />

                    {/* due_at only show when edit */}
                    {task && (
                        <>
                            <label className="block text-sm font-medium text-gray-700">
                                Due At
